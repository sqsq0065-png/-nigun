...
function updateClock(){const now=new Date(),e=$('#clock');if(e)e.textContent=new Intl.DateTimeFormat('he-IL',{hour:'2-digit',minute:'2-digit',hour12:false}).format(now);const d=$('#hebrewDate');if(d){try{d.textContent=new Intl.DateTimeFormat('he-IL-u-ca-hebrew',{weekday:'long',day:'numeric',month:'long',year:'numeric'}).format(now)}catch(_){d.textContent=new Intl.DateTimeFormat('he-IL',{weekday:'long',day:'numeric',month:'numeric'}).format(now)}}}

...
let trackSelectMode=false, selectedTracks=new Set();
...
function toggleTrackSelect(){trackSelectMode=!trackSelectMode;if(!trackSelectMode)selectedTracks.clear();$('#selectTracks').textContent=trackSelectMode?'סיום':'בחירה';$('#deleteSelected').classList.toggle('hidden',!trackSelectMode);render()}
async function deleteSelectedTracks(){const ids=[...selectedTracks];if(!ids.length)return toast('בחר שירים למחיקה');if(!confirm(`למחוק ${ids.length} שירים מהספרייה?`))return;const doomed=tracks.filter(t=>ids.includes(String(t.id)));if(navigator.onLine){const {error}=await sb.from('tracks').delete().in('id',ids);if(error)return toast('לא ניתן למחוק את השירים');const paths=doomed.flatMap(t=>[t.file_path,t.cover_path]).filter(Boolean);if(paths.length)await sb.storage.from('user-media').remove(paths).catch(()=>{});}tracks=tracks.filter(t=>!ids.includes(String(t.id)));selectedTracks.clear();trackSelectMode=false;$('#selectTracks').textContent='בחירה';$('#deleteSelected').classList.add('hidden');saveSnapshot();await render();toast(`${ids.length} שירים נמחקו`)}
...
async function init(){
 ...
 $('#selectTracks').onclick=toggleTrackSelect;$('#deleteSelected').onclick=deleteSelectedTracks;$('#closeArtist').onclick=()=>$('#artistModal').classList.remove('show');$('#artistModal').onclick=e=>{if(e.target.id==='artistModal')$('#artistModal').classList.remove('show')};$('#artistPicFile').onchange=uploadArtistPicture;$('#addRadio').onclick=addRadio;

...
async function artistPicture(name,list){const custom=getArtistPics()[name];if(custom)return offlineUrl(custom);const c=list.find(t=>t.cover_path)?.cover_path;return c?offlineUrl(c):'icon-512.png'}
...
function renderRadioStations(){const el=$('#radioList');if(!el)return;const a=getRadioStations();el.innerHTML=a.length?a.map((r,i)=>`<div class="radioRow"><div class="radioDot">R</div><div class="grow"><strong>${esc(r.name)}</strong><div class="sub">רדיו חי</div></div><button class="secondary" data-radio-play="${i}">נגן</button><button class="tinyBtn tinyDanger" data-radio-del="${i}">מחק</button></div>`).join(''):'<div class="sub">אין תחנות שמורות עדיין.</div>'}
function addRadio(){const name=$('#radioName').value.trim(),url=$('#radioUrl').value.trim();if(!name||!/^https?:\/\//i.test(url))return toast('כתוב שם תחנה וכתובת סטרים תקינה');const a=getRadioStations();a.push({name,url});saveRadioStations(a);$('#radioName').value='';$('#radioUrl').value='';renderRadioStations();toast('התחנה נוספה')}
async function playRadio(i){const r=getRadioStations()[i];if(!r)return;video.pause();audio.pause();currentMedia=audio;currentId=null;queue=[];audio.src=r.url;$('#miniCover').src='icon-192.png';$('#miniTitle').textContent=r.name;$('#miniArtist').textContent='רדיו חי';$('#mini').classList.add('show');try{await audio.play()}catch(e){toast('התחנה לא הצליחה להתחיל. בדוק שכתובת הסטרים ישירה ותומכת בדפדפן.')}sync()}
...
async function init(){
 ...
 $('#closeArtist').onclick=()=>$('#artistModal').classList.remove('show');$('#artistModal').onclick=e=>{if(e.target.id==='artistModal')$('#artistModal').classList.remove('show')};$('#artistPicFile').onchange=uploadArtistPicture;$('#addRadio').onclick=addRadio;
